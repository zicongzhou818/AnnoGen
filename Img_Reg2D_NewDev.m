function [pos_x,pos_y,T_temp,U1,U2]=Img_Reg2D_NewDev(T,R,N)
m=N;n=N;
%% Image registration 
%% step-0
...
tstep_ratio = 1e-11;% tstep_ratio: stop when tstep less than tstep_ratio
tstep_up=1.05;% tstep_up: magnification factor when ssd decreases
tstep_down=0.95;% tstep_down: reduction factor when doesn't ssd decrease
ite_max=5e2;% ite_max: preset maximum number of iterations
...
% display([' tstep: ',num2str(tstep),' ssd: ',num2str(ssd_old),' ratio: ',num2str(ratio)]);
%% Step-9 Main loop
...
while  (tstep>tstep_ratio) && (ii<ite_max) && (ratio>ratiotol) 
    ...
    if Better  % --(PS: Better holds for the first step in "while" loop)
        ...
        % w1,w2 
        lap_a1_interior=b1(2:m-1, 2:n-1);
        lap_a2_interior=b2(2:m-1, 2:n-1);
        w1=Poisson_FFT(lap_a1_interior);
        w2=Poisson_FFT(lap_a2_interior);
    end
    %% Step-3 Update F
    f1_new=f1-(w1)*tstep;
    f2_new=f2-(w2)*tstep;
    %% Step-4 Solve for U, where lap(U)=F
    intU1=Poisson_FFT(f1_new);
    intU2=Poisson_FFT(f2_new);
